import { APP_SHARED_DEPS } from '@naxi/extensions';
import { generateExtensionsEntrypoint, resolveFsExtensions, resolveModuleExtensions } from '@naxi/extensions/node';
...
export default defineConfig({
	...
	plugins: [
		...
		{
			name: 'watch-naxi-dependencies',
			configureServer: (server) => {
				server.watcher.options = {
					...server.watcher.options,
					ignored: [/node_modules\/(?!@naxi\/).*/, '**/.git/**'],
				};
			},
		},
		vueDevtools(),
...
function naxiExtensions() {
	const virtualExtensionsId = '@naxi-extensions';

	let extensionsEntrypoint = null;

	return [
		{
			name: 'naxi-extensions-serve',
			apply: 'serve',
			config: () => ({
				optimizeDeps: {
