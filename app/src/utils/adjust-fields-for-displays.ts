		...
		.map((fieldKey) => {
			...
			if (fieldKeys) {
				return fieldKeys.map((fieldKey) => {
					 ...
					 * naxi_files. The thumbnail itself isn't a real field, but shows the thumbnail based
					 * on the other available fields (like ID, title, and type).
					 */
					if (fieldKey.includes('$thumbnail') && field.collection === 'naxi_files') {
						return fieldKey
							.split('.')
