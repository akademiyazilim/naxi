...
import { LocalType, Relation } from '@naxi/types';
...
export function getLocalTypeForField(collection: string, field: string): LocalType | null {
	...
	if (relations.length === 1) {
		...
		if (relation.related_collection === 'naxi_files' && relation.related_collection !== collection) return 'file';
	...
	if (relations.length === 2) {
		...
		if (relations[0].related_collection === 'naxi_files' || relations[1].related_collection === 'naxi_files') {
			return 'files';
		} else {
			return 'm2m';
		}
