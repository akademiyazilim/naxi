...
import type { Panel } from '@naxi/extensions';
import { isSystemCollection } from '@naxi/system-data';
import type { Item } from '@naxi/types';
import { applyOptionsData, getSimpleHash, toArray } from '@naxi/utils';
...
export const useInsightsStore = defineStore('insightsStore', () => {
	...
	async function hydrate() {
		...
		if (hasPermission('naxi_dashboards', 'read') && hasPermission('naxi_panels', 'read')) {
			try {
