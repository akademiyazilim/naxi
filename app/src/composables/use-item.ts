...
import { Alterations, Field, Item, PrimaryKey, Query, Relation } from '@naxi/types';
...
export function useItem<T extends Item>(
	...
	function setItemValueToResponse(response: AxiosResponse) {
		if (
			(isSystemCollection(collection.value) && collection.value !== 'naxi_collections') ||
			(collection.value === 'naxi_collections' && isSystemCollection(response.data.data.collection ?? ''))
		) {
