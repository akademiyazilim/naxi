...
import { useCollection } from '@naxi/composables';
import { isSystemCollection } from '@naxi/system-data';
import { Field, PrimaryKey, Relation } from '@naxi/types';
import { getEndpoint } from '@naxi/utils';
...
function useFile() {
	const isnaxiFiles = computed(() => {
		return props.collection === 'naxi_files' || relatedCollection.value === 'naxi_files';
	});

	const file = computed(() => {
		if (isnaxiFiles.value === false || !initialValues.value) return null;
