...
import { useItems } from '@naxi/composables';
import { useAppStore } from '@naxi/stores';
import { Filter, Notification } from '@naxi/types';
...
watch(notificationsDrawerOpen, (open) => {
	...
	collection.value = open ? 'naxi_notifications' : null;
});
...
<template>
	...
	>
		...
		<template #actions>
			<search-input v-model="search" v-model:filter="filter" collection="naxi_notifications" />
