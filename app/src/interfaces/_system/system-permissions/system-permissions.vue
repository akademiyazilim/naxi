...
import { PERMISSION_ACTIONS } from '@naxi/constants';
import { appAccessMinimalPermissions, isSystemCollection } from '@naxi/system-data';
import { type Alterations, Filter, Permission, PermissionsAction } from '@naxi/types';
	...
	async function fetchPermissions() {
		...
		try {
			...
			if (itemId.value !== '+') {
				...
				const response = await api.get(getEndpoint('naxi_permissions'), {
					params: {
						fields: '*',
						filter,
						page: 0,
