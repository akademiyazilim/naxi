...
const { layout, layoutOptions, layoutQuery, filter, search, resetPreset } = usePreset(ref('naxi_users'));
...
} = useCollectionPermissions('naxi_users');

const { readAllowed: rolesReadAllowed } = useCollectionPermissions('naxi_roles');
...
<template>
	...
	>
		...
		>
			...
			<template #actions>
				<search-input v-model="search" v-model:filter="filter" collection="naxi_users" />
