...
const { createAllowed, updateAllowed, deleteAllowed } = useCollectionPermissions('naxi_dashboards');
...
function exportDasboard(ids: string[]) {
	const endpoint = getEndpoint('naxi_dashboards');
...
<template>
	<private-view :title="t('insights')">
		...
		<template #sidebar>
			...
			<basic-import-sidebar-detail collection="naxi_dashboards" @refresh="refresh" />
		</template>
