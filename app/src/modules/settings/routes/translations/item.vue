...
const {
	...
	isSingleton,
	accountabilityScope,
} = useCollection('naxi_translations');
...
const {
	...
	refresh,
	validationErrors,
} = useItem(ref('naxi_translations'), primaryKey);
...
<template>
	...
	<private-view v-else :title="primaryKey === '+' ? t('create_custom_translation') : t('edit_custom_translation')">
		...
		<template #navigation>
			<SettingsNavigation current-collection="naxi_translations" />
		</template>
		...
		<template #sidebar>
			...
			<template v-if="isNew === false && loading === false && internalPrimaryKey">
				<revisions-drawer-detail
					...
					@revert="revert"
				/>
				<comments-sidebar-detail collection="naxi_translations" :primary-key="internalPrimaryKey" />
				<flow-sidebar-detail
					location="item"
					collection="naxi_translations"
					:primary-key="internalPrimaryKey"
