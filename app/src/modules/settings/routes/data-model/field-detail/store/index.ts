...
import type { DisplayConfig, InterfaceConfig } from '@naxi/extensions';
import type { Collection, DeepPartial, Field, LocalType, Relation } from '@naxi/types';
		...
		async startEditing(collection: string, field: string, localType?: LocalType) {
			...
			if (field !== '+') {
				...
				if (['files', 'm2m', 'translations', 'm2a'].includes(this.localType)) {
					// These types rely on naxi_relations fields being said, so meta should exist for these particular relations
