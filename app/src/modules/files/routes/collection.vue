...
import { useLayout } from '@naxi/composables';
import { mergeFilters } from '@naxi/utils';
...
const { layout, layoutOptions, layoutQuery, filter, search, resetPreset } = usePreset(ref('naxi_files'));
...
const {
	...
	createAllowed,
} = useCollectionPermissions('naxi_files');

const { createAllowed: createFolderAllowed } = useCollectionPermissions('naxi_folders');
...
<template>
	...
	>
		...
		>
			...
			<template #actions>
				<search-input v-model="search" v-model:filter="filter" collection="naxi_files" />
