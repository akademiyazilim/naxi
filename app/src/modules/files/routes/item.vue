...
import type { Field, File } from '@naxi/types';
...
const {
	...
	refresh,
	validationErrors,
} = useItem<File>(ref('naxi_files'), primaryKey);
	...
	<private-view v-else :title="loading || !item ? t('loading') : item.title">
		...
		<template #sidebar>
			...
			<revisions-drawer-detail
				...
				:primary-key="primaryKey"
			/>
			<comments-sidebar-detail v-if="isNew === false" collection="naxi_files" :primary-key="primaryKey" />
		</template>
	</private-view>
</template>
