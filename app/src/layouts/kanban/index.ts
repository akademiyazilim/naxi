...
import { useCollection, useFilterFields, useItems, useSync } from '@naxi/composables';
import { defineLayout } from '@naxi/extensions';
import { Field, User } from '@naxi/types';
import { getEndpoint, getRelationType, moveInArray } from '@naxi/utils';
	...
	setup(props, { emit }) {
				...
				if (junction !== undefined) {
					const related = relationsStore.relations.find(
						(relation) =>
							...
							relation.related_collection === 'naxi_users',
					);
			...
			file: (field) => {
				...
				const relation = relationsStore.relations.find((relation) => {
					return (
						relation.collection === props.collection &&
						relation.field === field.field &&
						relation.related_collection === 'naxi_files'
					);
				});
		...
		function useLayoutQuery() {
			...
			const fields = computed<string[]>(() => {
				...
				if (props.collection === 'naxi_files' && imageSource.value === '$thumbnail') {
