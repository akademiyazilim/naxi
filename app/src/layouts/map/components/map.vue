...
import type { ShowSelect } from '@naxi/extensions';
...
const boxSelectControl = new BoxSelectControl({
	...
	layers: ['__naxi_polygons', '__naxi_points', '__naxi_lines'],
});
...
function setupMap() {
	...
	map.on('load', () => {
		...
		const activeLayers = ['__naxi_polygons', '__naxi_points', '__naxi_lines'];
		...
		map.on('click', '__naxi_clusters', expandCluster);
		map.on('mousemove', '__naxi_clusters', hoverCluster);
		map.on('mouseleave', '__naxi_clusters', hoverCluster);
...
function updateData(newData: any) {
	const source = map.getSource('__naxi');
...
function updateSource(newSource: GeoJSONSource) {
	...
	if (map.getStyle().sources?.['__naxi']) {
		map.removeSource('__naxi');
	}

	map.addSource('__naxi', { ...newSource, data: props.data });
...
function updateSelection(newSelection?: (string | number)[], previousSelection?: (string | number)[]) {
	previousSelection?.forEach((id) => {
		map.setFeatureState({ id, source: '__naxi' }, { selected: false });
		map.removeFeatureState({ id, source: '__naxi' });
	});

	newSelection?.forEach((id) => {
		map.setFeatureState({ id, source: '__naxi' }, { selected: true });
	});
}
...
function updatePopup(event: MapLayerMouseEvent) {
	...
	if (previousId && featureChanged) {
		map.setFeatureState({ id: previousId, source: '__naxi' }, { hovered: false });
	}
	...
	if (feature && feature.properties) {
		...
		if (featureChanged) {
			map.setFeatureState({ id: feature.id, source: '__naxi' }, { hovered: true });
...
function expandCluster(event: MapLayerMouseEvent) {
	const features = map.queryRenderedFeatures(event.point, {
		layers: ['__naxi_clusters'],
	});

	const clusterId = features[0]?.properties?.cluster_id;
	const source = map.getSource('__naxi') as GeoJSONSource;
