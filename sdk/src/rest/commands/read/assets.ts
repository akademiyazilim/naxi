import type { naxiFile } from '../../../schema/file.js';
...
export const readAssetRaw =
	<Schema>(key: naxiFile<Schema>['id'], query?: AssetsQuery): RestCommand<ReadableStream<Uint8Array>, Schema> =>
	() => {
...
export const readAssetBlob =
	<Schema>(key: naxiFile<Schema>['id'], query?: AssetsQuery): RestCommand<Blob, Schema> =>
	() => {
		throwIfEmpty(String(key), 'Key cannot be empty');
...
export const readAssetArrayBuffer =
	<Schema>(key: naxiFile<Schema>['id'], query?: AssetsQuery): RestCommand<ArrayBuffer, Schema> =>
	() => {
		throwIfEmpty(String(key), 'Key cannot be empty');
