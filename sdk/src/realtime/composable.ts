...
/**
 * Creates a client to communicate with a naxi REST WebSocket.
 *
 * @param config The optional configuration.
 *
 * @returns A naxi realtime client.
 */
export function realtime(config: WebSocketConfig = {}) {
	return <Schema>(client: naxiClient<Schema>) => {
		...
		const debug = (level: keyof ConsoleInterface, ...data: any[]) =>
			config.debug && client.globals.logger[level]('[naxi SDK]', ...data);
