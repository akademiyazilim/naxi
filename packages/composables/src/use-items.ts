import type { Item, Query } from '@naxi/types';
import { getEndpoint, moveInArray } from '@naxi/utils';
...
export function useItems(collection: Ref<string | null>, query: ComputedQuery): UsableItems {
	...
	async function getItems() {
		...
		try {
			 ...
			 * able to render out the naxi_files collection (file library) using regular layouts
			 *
			 * Layouts expect the file to be a m2o of a `file` type, however, naxi_files is the
			 ...
			 */
			if (collection.value === 'naxi_files') {
