import type { Field, Filter } from '@naxi/types';
...
test('should append $thumbnail to fetched items when collection is naxi_files', async () => {
	vi.mocked(useCollection).mockReturnValueOnce({ primaryKeyField: computed(() => null) } as any);

	const collection = ref('naxi_files');
