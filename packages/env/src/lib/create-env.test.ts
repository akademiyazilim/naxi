...
import { isnaxiVariable } from '../utils/is-naxi-variable.js';
...
vi.mock('../utils/is-naxi-variable.js');
...
describe('File based configuration', () => {
	beforeEach(() => {
		...
		vi.mocked(isnaxiVariable).mockImplementation((key) => {
			return key === 'PROCESS_FILE';
		});
...
test('Passthrough file variables that are not naxi configuration flags', () => {
	vi.mocked(readConfigurationFromFile).mockReturnValue({
		TEST_FILE: './test/path',
	});

	vi.mocked(isnaxiVariable).mockImplementation(() => {
		return false;
	});
