import { randomAlpha, randomInteger } from '@naxi/random';
...
test('Returns enhanced error object', () => {
	...
	expect(error.name).toBe('naxiError');
...
test('Overrides toString', () => {
	...
	expect(error.toString()).toBe(`naxiError [${sample.code.toUpperCase()}]: ${sample.message}`);
});
