...
program
	.version(pkg.version)
	.arguments('<directory>')
	.description('Create a new naxi project')
	.action(create)
	.parse(process.argv);
...
async function create(directory) {
	...
	// Let's get into the naxi mood while waiting
	const bunnyFrames = [];
	const numOfSpaces = 3;
	...
	spinner.start(chalk.bold('Installing naxi'));
	...
	try {
		await execa('npm', ['install', 'naxi', '--omit=dev'], {
			cwd: rootPath,
			stdin: 'ignore',
		});
	} catch (err) {
