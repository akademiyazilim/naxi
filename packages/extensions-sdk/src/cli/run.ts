...
program.name('naxi-extension').usage('[command] [options]');
program.version(getSdkVersion(), '-v, --version');

program
	.command('create')
	.arguments('<type> <name>')
	.description('Scaffold a new naxi extension')
...
program
	.command('add')
	.description('Add entries to an existing naxi extension')
	.option('--no-install', 'skip dependency (re)installation after adding extension')
	.action(add);

program
	.command('build')
	.description('Bundle a naxi extension to a single entrypoint')
...
program
	.command('link')
	.description('Creates a symlink to the extension in the naxi extensions folder')
	.argument('<path>', 'path to the extension folder of naxi')
	.action(link);
