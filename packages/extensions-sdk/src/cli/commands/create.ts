...
async function createBundleExtension({
	...
	const spinner = ora(chalk.bold('Scaffolding naxi extension...')).start();
...
function getPackageManifest(name: string, options: ExtensionOptions, deps: Record<string, string>) {
	const packageManifest: Record<string, any> = {
		...
		icon: 'extension',
		version: '1.0.0',
		keywords: ['naxi', 'naxi-extension', `naxi-extension-${options.type}`],
		type: 'module',
		files: ['dist'],
		[EXTENSION_PKG_KEY]: options,
		scripts: {
			build: 'naxi-extension build',
			dev: 'naxi-extension build -w --no-minify',
			link: 'naxi-extension link',
		},
		devDependencies: deps,
	};

	if (options.type === 'bundle') {
		packageManifest['scripts']['add'] = 'naxi-extension add';
	}
