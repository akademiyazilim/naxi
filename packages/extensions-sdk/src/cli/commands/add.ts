...
import { isIn, isTypeIn } from '@naxi/utils';
import { pathToRelativeUrl } from '@naxi/utils/node';
...
export default async function add(options: AddOptions): Promise<void> {
	...
	if (!(await fse.pathExists(packagePath))) {
		log(`Current directory is not a valid naxi extension:`, 'error');
	...
	if (extensionOptions.type === 'bundle') {
		...
		const spinner = ora(chalk.bold('Modifying naxi extension...')).start();
		...
		const newExtensionManifest = {
			...extensionManifest,
			name: extensionName,
			keywords: ['naxi', 'naxi-extension', 'naxi-extension-bundle'],
			[EXTENSION_PKG_KEY]: newExtensionOptions,
			devDependencies: {
				...extensionManifest.devDependencies,
				...(await getExtensionDevDeps(
