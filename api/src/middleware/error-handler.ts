import { ErrorCode, InternalServerError, isnaxiError } from '@naxi/errors';
import type { DeepPartial } from '@naxi/types';
import { isObject } from '@naxi/utils';
import { getNodeEnv } from '@naxi/utils/node';
...
export const errorHandler = asyncErrorHandler(async (err, req, res) => {
	...
	for (const error of receivedErrors) {
		...
		if (isnaxiError(error)) {
			...
			if (isnaxiError(error, ErrorCode.MethodNotAllowed)) {
