...
test('Runs and returns basic counts', async () => {
	...
	expect(getItemCount).toHaveBeenCalledWith(mockDb, [
		...
		{ collection: 'naxi_flows', where: ['status', '=', 'active'] },
		{ collection: 'naxi_roles' },
		{ collection: 'naxi_shares' },
	]);

	expect(report.dashboards).toBe(mockItemCount.naxi_dashboards);
	expect(report.files).toBe(mockItemCount.naxi_files);
	expect(report.flows).toBe(mockItemCount.naxi_flows);
	expect(report.roles).toBe(mockItemCount.naxi_roles);
	expect(report.shares).toBe(mockItemCount.naxi_shares);
});
