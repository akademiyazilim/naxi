...
import { ErrorCode, ForbiddenError, isnaxiError, RouteNotFoundError } from '@naxi/errors';
import { EXTENSION_TYPES } from '@naxi/extensions';
import {
	account,
	describe,
	list,
	...
	type ListOptions,
	type ListQuery,
} from '@naxi/extensions-registry';
import type { FieldFilter } from '@naxi/types';
import { isIn } from '@naxi/utils';
...
router.use(useCollection('naxi_extensions'));
	...
	asyncHandler(async (req, res, next) => {
		...
		try {
			...
			if (error instanceof ExtensionReadError) {
				...
				if (isnaxiError(finalError, ErrorCode.Forbidden)) {
					return next();
				}
			}
