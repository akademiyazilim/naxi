...
import { ErrorCode, InvalidPayloadError, isnaxiError } from '@naxi/errors';
import formatTitle from '@naxi/format-title';
import type { BusboyFileStream, PrimaryKey } from '@naxi/types';
...
router.use(useCollection('naxi_files'));
...
export const multipartHandler: RequestHandler = (req, res, next) => {
	 ...
	 * the row in naxi_files async during the upload of the actual file.
	 */
...
router.post(
	...
	asyncHandler(async (req, res, next) => {
		...
		try {
			...
			if (isnaxiError(error, ErrorCode.Forbidden)) {
				return next();
			}
...
const readHandler = asyncHandler(async (req, res, next) => {
	...
	const meta = await metaService.getMetaForQuery('naxi_files', req.sanitizedQuery);
