...
describe('should fail on invalid snapshot schema', () => {
	...
	test('invalid schema', () => {
		const snapshot = { version: 1, naxi: '10.0.0', collections: {} } as Snapshot;
...
describe('should require force option on version / vendor mismatch', () => {
	test('naxi version mismatch', () => {
		const snapshot = { version: 1, naxi: '9.26.0' } as Snapshot;

		expect(() => validateSnapshot(snapshot)).toThrowError(
			"Provided snapshot's naxi version 9.26.0 does not match the current instance's version 10.0.0",
		);
	});

	test('db vendor mismatch', () => {
		const snapshot = { version: 1, naxi: '10.0.0', vendor: 'postgres' } as Snapshot;
...
test('should allow bypass on version / vendor mismatch via force option ', () => {
	const snapshot = { version: 1, naxi: '9.26.0', vendor: 'postgres' } as Snapshot;
