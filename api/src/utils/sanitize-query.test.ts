...
import { parseFilter, parseJSON } from '@naxi/utils';
import { afterEach, beforeEach, describe, expect, test, vi } from 'vitest';
import { sanitizeQuery } from './sanitize-query.js';

// This is required because logger uses global env which is imported before the tests run. Can be
// reduce to just mock the file when logger is also using useLogger everywhere @TODO
vi.mock('@naxi/env', () => ({ useEnv: vi.fn().mockReturnValue({}) }));

vi.mock('@naxi/utils', async () => {
	const actual = await vi.importActual<typeof import('@naxi/utils')>('@naxi/utils');
