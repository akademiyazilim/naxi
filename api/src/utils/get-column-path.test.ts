import type { DeepPartial } from '@naxi/types';
import { expect, test } from 'vitest';
import { InvalidQueryError } from '@naxi/errors';
...
/*
{
  ...
  collection: 'articles',
  aliasMap: {
    author: { alias: 'grenv', collection: 'naxi_users' },
	'author.role': { alias: 'ljnsv', collection: 'naxi_roles' },
  },
  relations: []
...
test('Returns correct alias for m2o', () => {
	const input: DeepPartial<ColPathProps> = {
		...
		collection: 'articles',
		aliasMap: {
			author: { alias: 'ljnsv', collection: 'naxi_users' },
			'author.role': { alias: 'grenv', collection: 'naxi_roles' },
		},
		relations: [
			{
				collection: 'articles',
				field: 'author',
				related_collection: 'naxi_users',
				meta: null,
				schema: null,
			},
			{
	...
	expect(result.targetCollection).toBe('naxi_roles');
});
...
test('Returns correct alias when there are multiple joins to the same table', () => {
	const input: DeepPartial<ColPathProps> = {
		...
		aliasMap: {
			author: { alias: 'ljnsv', collection: 'naxi_users' },
			'author.role': { alias: 'grenv', collection: 'naxi_roles' },
			'author.secondary_role': { alias: 'psgwn', collection: 'naxi_roles' },
		},
		relations: [
			{
				collection: 'articles',
