import { TYPES } from '@naxi/constants';
import Joi from 'joi';
...
export function validateSnapshot(snapshot: Snapshot, force = false) {
	...
	if (snapshot.naxi !== version) {
		throw new InvalidPayloadError({
			reason: `Provided snapshot's naxi version ${snapshot.naxi} does not match the current instance's version ${version}. You can bypass this check by passing the "force" query parameter`,
		});
	}
