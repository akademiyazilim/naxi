...
import type { Filter, Permission, Query } from '@naxi/types';
...
export function getDBQuery(
	...
	if (sortRecords) {
		...
		if (needsInnerQuery) {
			...
			if (hasMultiRelationalSort) {
				...
				// Start order by with naxi_row_number. The naxi_row_number is derived from a window function that
				...
				// Since the only relevant result is the first row of this partition, adding the naxi_row_number to the
				// order by here ensures that all rows with a naxi_row_number = 1 show up first in the inner query result,
				// and are correctly truncated by the limit, but not earlier.
				orderByString = `?? asc, ${orderByString}`;
				orderByFields.unshift(knex.ref('naxi_row_number'));
			}
	...
	if (sortRecords) {
		...
		if (hasMultiRelationalSort) {
			wrapperQuery.where('inner.naxi_row_number', '=', 1);
