...
import type { SchemaInspector } from '@naxi/schema';
...
export async function isInstalled(): Promise<boolean> {
	...
	// The existence of a naxi_collections table alone isn't a "proper" check to see if everything
	// is installed correctly of course, but it's safe enough to assume that this collection only
	// exists when naxi is properly installed.
	return await inspector.hasTable('naxi_collections');
}
...
export async function validateMigrations(): Promise<boolean> {
	...
	try {
		...
		const completedVersions = (await database.select('version').from('naxi_migrations')).map(
			({ version }) => version,
		);
