...
export async function up(knex: Knex): Promise<void> {
	const groupsInUse = await knex.select('id', 'group').from('naxi_fields').whereNotNull('group');

	const groupIDs: number[] = uniq(groupsInUse.map(({ group }) => group));

	const groupFields = await knex.select('id', 'field').from('naxi_fields').whereIn('id', groupIDs);
...
export async function down(knex: Knex): Promise<void> {
	...
	for (const { collection, group } of fieldsThatUseAGroup) {
		const { id } = await knex.select('id').from('naxi_fields').where({ collection, field: group }).first();
	...
	await knex.schema.alterTable('naxi_fields', (table) => {
		table.integer('group').references('id').inTable('naxi_fields');
	});
