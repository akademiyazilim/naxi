...
export async function up(knex: Knex): Promise<void> {
	await knex.schema.createTable('naxi_versions', (table) => {
	...
	await knex.schema.alterTable('naxi_revisions', (table) => {
		table.uuid('version').references('id').inTable('naxi_versions').onDelete('CASCADE');
	});
}
...
export async function down(knex: Knex): Promise<void> {
	...
	await knex.schema.dropTable('naxi_versions');
}
