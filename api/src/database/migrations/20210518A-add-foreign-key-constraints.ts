...
import type { RelationMeta } from '@naxi/types';
...
export async function up(knex: Knex): Promise<void> {
	...
	if (corruptedRelations.length > 0) {
		logger.warn(
			`Encountered one or more corrupted relationships. Please check the following rows in "naxi_relations": ${corruptedRelations.join(
				', ',
			)}`,
		);
	}
