...
export async function up(knex: Knex): Promise<void> {
	if (getDatabaseClient(knex) === 'mysql') {
		...
		await knex.schema.raw(
			'ALTER TABLE `naxi_files` CHANGE `uploaded_on` `created_on` TIMESTAMP NOT NULL DEFAULT current_timestamp();',
		);
	} else {
	...
	await knex('naxi_files').update('uploaded_on', knex.ref('created_on'));
}
...
export async function down(knex: Knex): Promise<void> {
	...
	if (getDatabaseClient(knex) === 'mysql') {
		await knex.schema.raw(
			'ALTER TABLE `naxi_files` CHANGE `created_on` `uploaded_on` TIMESTAMP NOT NULL DEFAULT current_timestamp();',
		);
	} else {
		await knex.schema.alterTable('naxi_files', (table) => {
