...
export async function up(knex: Knex): Promise<void> {
	...
	await knex.schema.alterTable('naxi_shares', (table) => {
...
/**
 * Temporarily drop foreign key constraint for MySQL instances, see https://github.com/naxi/naxi/issues/19399
 */
async function dropConstraint(knex: Knex) {
	...
	const foreignKeys = await inspector.foreignKeys('naxi_shares');
	...
	if (constraintName && collectionForeignKeys.length === 1) {
		...
		logger.warn(`Unexpected number of foreign key constraints on 'naxi_shares.collection':`);
...
async function recreateConstraint(knex: Knex) {
	return knex.schema.alterTable('naxi_shares', async (table) => {
		table.foreign('collection').references('naxi_collections.collection').onDelete('CASCADE');
	});
}
