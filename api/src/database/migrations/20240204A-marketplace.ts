import { resolvePackage } from '@naxi/utils/node';
...
export async function up(knex: Knex): Promise<void> {
	await knex.schema.alterTable('naxi_extensions', (table) => {
	...
	const installedExtensions = await knex.select('name').from('naxi_extensions');
	...
	for (const { name } of installedExtensions) {
		...
		if (name.includes(':')) {
			await knex('naxi_extensions').delete().where({ name });
		} else {
			const id = randomUUID();
			...
			await knex('naxi_extensions').update({ id, source, folder: name }).where({ name });
	...
	await knex.transaction(async (trx) => {
		await trx.schema.alterTable('naxi_extensions', (table) => {
			table.dropPrimary();
			table.primary(['id']);
		});
	});
...
export async function down(knex: Knex): Promise<void> {
	...
	const installedExtensions = await knex.select(['id', 'folder', 'bundle']).from('naxi_extensions');
	...
	for (const { id, folder, bundle, source } of installedExtensions) {
		if (source === 'registry') {
			await knex('naxi_extensions').delete().where({ id });
			continue;
		}
		...
		await knex('naxi_extensions').update({ name }).where({ id });
	}
