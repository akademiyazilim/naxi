...
export async function up(knex: Knex): Promise<void> {
	await knex.schema.createTable('naxi_translations', (table) => {
	...
	const data = await knex.select('translation_strings', 'id').from('naxi_settings').first();
	...
	if (data?.translation_strings && data?.id) {
		...
		for (const item of newTranslationStrings) {
			await knex('naxi_translations').insert(item);
		}
	}
...
export async function down(knex: Knex): Promise<void> {
	const data = await knex.select('language', 'key', 'value').from('naxi_translations');
	const settingsId = await knex.select('id').from('naxi_settings').first();
	...
	await knex.schema.dropTable('naxi_translations');
}
