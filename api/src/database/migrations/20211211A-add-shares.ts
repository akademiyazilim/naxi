...
export async function up(knex: Knex): Promise<void> {
	await knex.schema.createTable('naxi_shares', (table) => {
		table.uuid('id').primary().notNullable();
		table.string('name');
		table.string('collection', 64).references('collection').inTable('naxi_collections').onDelete('CASCADE');
		table.string('item');
		table.uuid('role').references('id').inTable('naxi_roles').onDelete('CASCADE');
	...
	await knex.schema.alterTable('naxi_sessions', (table) => {
		table.dropColumn('data');
	});

	await knex.schema.alterTable('naxi_sessions', (table) => {
		table.setNullable('user');
		table.uuid('share').references('id').inTable('naxi_shares').onDelete('CASCADE');
	});
}
...
export async function down(knex: Knex): Promise<void> {
	...
	await knex.schema.dropTable('naxi_shares');
}
