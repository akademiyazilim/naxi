import { processChunk, toBoolean } from '@naxi/utils';
...
export async function up(knex: Knex) {
	...
	if (await knex.schema.hasTable('naxi_policies')) {
		return;
	}
	...
	await knex.schema.createTable('naxi_policies', (table) => {
	...
	if (roles.length > 0) {
		await processChunk(roles, 100, async (chunk) => {
			await knex('naxi_policies').insert(chunk);
		});
	}
	...
	await knex.schema.alterTable('naxi_roles', (table) => {
		...
		table.uuid('parent').references('naxi_roles.id');
	});
	...
	try {
		...
		const foreignKeys = await inspector.foreignKeys('naxi_permissions');

		const foreignConstraint =
			foreignKeys.find((foreign) => foreign.foreign_key_table === 'naxi_roles' && foreign.column === 'role')
		...
		logger.warn('Failed to drop foreign key constraint on `role` column in `naxi_permissions` table');
	}

	await knex('naxi_permissions')
		.update({
			role: PUBLIC_POLICY_ID,
		})
		.whereNull('role');

	await knex('naxi_permissions').update({
	...
	await knex.schema.alterTable('naxi_permissions', (table) => {
		...
		table.foreign('policy').references('naxi_policies.id').onDelete('CASCADE');
	});
	...
	// well, which would require the O2A type to exist in naxi which currently doesn't.
	// Shouldn't be the end of the world here, as we know we're only attaching policies to two other
	// collections.

	await knex.schema.createTable('naxi_access', (table) => {
		table.uuid('id').primary();
		table.uuid('role').references('naxi_roles.id').nullable().onDelete('CASCADE');
		table.uuid('user').references('naxi_users.id').nullable().onDelete('CASCADE');
		table.uuid('policy').references('naxi_policies.id').notNullable().onDelete('CASCADE');
		table.integer('sort');
	});
	...
	await processChunk(policyAttachments, 100, async (chunk) => {
		await knex('naxi_access').insert(chunk);
	});

	await knex('naxi_access').insert({
		id: randomUUID(),
		role: null,
		user: null,
...
export async function down(knex: Knex) {
	/////////////////////////////////////////////////////////////////////////////////////////////////
	// Reinstate access control fields on naxi roles
	...
	// fetch all roles
	const roles: Array<{ id: string | null }> = await knex.select('id').from('naxi_roles');
	...
	for (const role of roles) {
		...
		if (role.id !== null) {
			...
			await knex('naxi_roles').update(roleAccess).where({ id: role.id });
		}
	...
	//  as soon as we reference naxi_roles in naxi_permissions again

	/////////////////////////////////////////////////////////////////////////////////////////////////
	// Drop policy attachments

	await knex.schema.dropTable('naxi_access');
	...
	await processChunk(originalPermissions, 100, async (chunk) => {
		await knex('naxi_permissions').delete(chunk);
	});

	// insert all v10 permissions
	await processChunk(rolePermissions, 100, async (chunk) => {
		await knex('naxi_permissions').insert(chunk);
	});

	await knex.schema.alterTable('naxi_permissions', (table) => {
		table.uuid('role').references('naxi_roles.id').alter();
	...
	await knex.schema.dropTable('naxi_policies');
}
