...
describe('run', () => {
	...
	describe('when passed the argument up', () => {
		it('returns "Nothing To Upgrade" if no naxi_migrations', async () => {
			tracker.on.select('naxi_migrations').response(['Empty']);
		...
		it('returns "Method implemented in the dialect driver" if no naxi_migrations', async () => {
			tracker.on.select('naxi_migrations').response([]);
		...
		it('returns undefined if the migration is successful', async () => {
			...
			tracker.on.delete('naxi_relations').response([]);
			tracker.on.insert('naxi_migrations').response(['Remove System Relations', '20201029A']);
	...
	describe('when passed the argument down', () => {
		it('returns "Nothing To downgrade" if no valid naxi_migrations', async () => {
			tracker.on.select('naxi_migrations').response(['Empty']);
