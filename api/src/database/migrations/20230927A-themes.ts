...
export async function up(knex: Knex): Promise<void> {
	...
	await knex('naxi_users').update({ appearance: 'dark' }).where({ theme: 'dark' });
	await knex('naxi_users').update({ appearance: 'light' }).where({ theme: 'light' });
	...
	await knex('naxi_settings').update({ project_color: '#6644ff' }).whereNull('project_color');

	await knex.schema.alterTable('naxi_settings', (table) => {
		table.string('project_color').defaultTo('#6644FF').notNullable().alter();
		table.uuid('public_favicon').references('naxi_files.id');
