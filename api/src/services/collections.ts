...
import { ForbiddenError, InvalidPayloadError } from '@naxi/errors';
import type { SchemaInspector, Table } from '@naxi/schema';
import { createInspector } from '@naxi/schema';
import { systemCollectionRows, type BaseCollectionMeta } from '@naxi/system-data';
import type { Accountability, FieldMeta, RawField, SchemaOverview } from '@naxi/types';
import { addFieldFlag } from '@naxi/utils';
...
export class CollectionsService {
	...
	async createOne(payload: RawCollection, opts?: MutationOptions): Promise<string> {
		...
		if (payload.collection.startsWith('naxi_')) {
			throw new InvalidPayloadError({ reason: `Collections can't start with "naxi_"` });
		}
		...
		try {
			const existingCollections: string[] = [
				...((await this.knex.select('collection').from('naxi_collections'))?.map(({ collection }) => collection) ??
					[]),
			...
			await transaction(this.knex, async (trx) => {
				if (payload.schema) {
					// naxi heavily relies on the primary key of a collection, so we have to make sure that
					...
					const fieldItemsService = new ItemsService('naxi_fields', {
						knex: trx,
				...
				if (payload.meta) {
					const collectionsItemsService = new ItemsService('naxi_collections', {
						knex: trx,
						accountability: this.accountability,
						schema: this.schema,
					});
	...
	async deleteOne(collectionKey: string, opts?: MutationOptions): Promise<string> {
		...
		try {
			...
			await transaction(this.knex, async (trx) => {
				...
				await trx('naxi_collections').update({ group: null }).where({ group: collectionKey });
				...
				if (collectionToBeDeleted!.schema) {
					...
					await trx('naxi_fields').delete().where('collection', '=', collectionKey);
					await trx('naxi_presets').delete().where('collection', '=', collectionKey);
					...
					if (revisionsToDelete.length > 0) {
						...
						for (const keys of chunks) {
							await trx('naxi_revisions').update({ parent: null }).whereIn('parent', keys);
						}
					}

					await trx('naxi_revisions').delete().where('collection', '=', collectionKey);

					await trx('naxi_activity').delete().where('collection', '=', collectionKey);
					await trx('naxi_permissions').delete().where('collection', '=', collectionKey);
					await trx('naxi_relations').delete().where({ many_collection: collectionKey });
