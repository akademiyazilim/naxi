...
import type { Accountability, SchemaOverview } from '@naxi/types';
...
import type { AbstractServiceOptions, naxiTokenPayload, LoginResult, Session, User } from '../types/index.js';
...
export class AuthenticationService {
	...
	async login(
		...
		if (allowedAttempts !== null) {
			...
			try {
				...
				if (error instanceof RateLimiterRes && error.remainingPoints === 0) {
					await this.knex('naxi_users').update({ status: 'suspended' }).where({ id: user.id });
		...
		const tokenPayload: naxiTokenPayload = {
			id: user.id,
			role: user.role,
		...
		await this.knex('naxi_sessions').insert({
			token: refreshToken,
			user: user.id,
		...
		await this.knex('naxi_sessions').delete().where('expires', '<', new Date());
		...
		await this.knex('naxi_users').update({ last_access: new Date() }).where({ id: user.id });
	...
	async refresh(refreshToken: string, options?: Partial<{ session: boolean }>): Promise<LoginResult> {
		...
		const record = await this.knex
			...
			.from('naxi_sessions AS s')
			.leftJoin('naxi_users AS u', 's.user', 'u.id')
			.leftJoin('naxi_shares AS d', 's.share', 'd.id')
		...
		if (record.user_id && record.user_status !== 'active') {
			await this.knex('naxi_sessions').where({ token: refreshToken }).del();
		...
		if (record.user_id) {
			await this.knex('naxi_users').update({ last_access: new Date() }).where({ id: record.user_id });
		}
	...
	private async updateStatefulSession(
		...
		const updatedSession = await this.knex('naxi_sessions')
			.update(
				{
		...
		if (updatedSession.length === 0) {
			...
			const { next_token } = await this.knex('naxi_sessions')
				.select('next_token')
	...
	async logout(refreshToken: string): Promise<void> {
		const record = await this.knex
			...
			.from('naxi_sessions as s')
			.innerJoin('naxi_users as u', 's.user', 'u.id')
			.where('s.token', refreshToken)
			.first();
		...
		if (record) {
			...
			await this.knex.delete().from('naxi_sessions').where('token', refreshToken);
		}
	}
