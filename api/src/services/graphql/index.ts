import { FUNCTIONS } from '@naxi/constants';
import { useEnv } from '@naxi/env';
import { ErrorCode, ForbiddenError, InvalidPayloadError, isnaxiError, type naxiError } from '@naxi/errors';
...
import { parseFilterFunctionPath, toBoolean } from '@naxi/utils';
...
import isnaxiJWT from '../../utils/is-naxi-jwt.js';
...
const READ_ONLY = ['naxi_activity', 'naxi_revisions'];
...
export class GraphQLService {
					...
					fields: Object.values(collection.fields).reduce(
						(acc, field) => {
							...
							if (collection.primary === field.field && fieldIsInconsistent === false) {
								// permissions IDs need to be nullable https://github.com/naxi/naxi/issues/20509
								if (collection.collection === 'naxi_permissions') {
									type = GraphQLID;
	 ...
	 * naxi' query structure which is then executed by the services.
	 */
	async resolveQuery(info: GraphQLResolveInfo): Promise<Partial<Item> | null> {
		let collection = info.fieldName;
		if (this.scope === 'system') collection = `naxi_${collection}`;
	 ...
	 * whole nested tree into one big query using naxi' own query resolver, we want to have a nested structure of
	 * arguments for the whole resolving tree, which can later be transformed into naxi' AST using `deep`.
	...
	/**
	 * Get a naxi Query object from the parsed arguments (rawQuery) and GraphQL AST selectionSet. Converts SelectionSet into
	 * naxi' `fields` query for use in the resolver. Also applies variables where appropriate.
	 */
	getQuery(
		rawQuery: Query,
	...
	/**
	 * Replace functions from GraphQL format to naxi-Filter format
	 */
	...
	/**
	 * Convert naxi-Exception into a GraphQL format, so it can be returned by GraphQL properly.
	 */
	formatError(error: naxiError | naxiError[]): GraphQLError {
	...
	injectSystemResolvers(
			...
			auth_password_request: {
				...
				resolve: async (_, args, { req }) => {
					...
					try {
						...
						if (isnaxiError(err, ErrorCode.InvalidPayload)) {
							throw err;
						}
					}
		...
		if ('naxi_collections' in schema.read.collections) {
			Collection.addFields({
				collection: GraphQLString,
				meta: schemaComposer.createObjectTC({
					name: 'naxi_collections_meta',
					fields: Object.values(schema.read.collections['naxi_collections']!.fields).reduce(
						(acc, field) => {
							acc[field.field] = {
								type: field.nullable
		...
		if ('naxi_fields' in schema.read.collections) {
			Field.addFields({
				collection: GraphQLString,
				field: GraphQLString,
				type: GraphQLString,
				meta: schemaComposer.createObjectTC({
					name: 'naxi_fields_meta',
					fields: Object.values(schema.read.collections['naxi_fields']!.fields).reduce(
						(acc, field) => {
							acc[field.field] = {
								type: field.nullable
									? getGraphQLType(field.type, field.special)
		...
		if ('naxi_relations' in schema.read.collections) {
			Relation.addFields({
				collection: GraphQLString,
				field: GraphQLString,
				related_collection: GraphQLString,
				schema: schemaComposer.createObjectTC({
					name: 'naxi_relations_schema',
					fields: {
				...
				meta: schemaComposer.createObjectTC({
					name: 'naxi_relations_meta',
					fields: Object.values(schema.read.collections['naxi_relations']!.fields).reduce(
						(acc, field) => {
							acc[field.field] = {
		...
		if (this.accountability?.admin === true) {
			schemaComposer.Mutation.addFields({
				create_collections_item: {
					type: Collection,
					args: {
						data: toInputObjectType(Collection.clone('create_naxi_collections'), {
							postfix: '_input',
						}).addFields({
							fields: [
								toInputObjectType(Field.clone('create_naxi_collections_fields'), { postfix: '_input' }).NonNull,
							],
						}).NonNull,
					},
					resolve: async (_, args) => {
				...
				update_collections_item: {
					...
					args: {
						collection: new GraphQLNonNull(GraphQLString),
						data: toInputObjectType(Collection.clone('update_naxi_collections'), {
							postfix: '_input',
			...
			schemaComposer.Mutation.addFields({
				create_fields_item: {
					...
					args: {
						collection: new GraphQLNonNull(GraphQLString),
						data: toInputObjectType(Field.clone('create_naxi_fields'), { postfix: '_input' }).NonNull,
					},
					resolve: async (_, args) => {
						const service = new FieldsService({
							accountability: this.accountability,
				...
				update_fields_item: {
					...
					args: {
						collection: new GraphQLNonNull(GraphQLString),
						field: new GraphQLNonNull(GraphQLString),
						data: toInputObjectType(Field.clone('update_naxi_fields'), { postfix: '_input' }).NonNull,
					},
					resolve: async (_, args) => {
						const service = new FieldsService({
							accountability: this.accountability,
			...
			schemaComposer.Mutation.addFields({
				create_relations_item: {
					type: Relation,
					args: {
						data: toInputObjectType(Relation.clone('create_naxi_relations'), { postfix: '_input' }).NonNull,
					},
					resolve: async (_, args) => {
				...
				update_relations_item: {
					...
					args: {
						collection: new GraphQLNonNull(GraphQLString),
						field: new GraphQLNonNull(GraphQLString),
						data: toInputObjectType(Relation.clone('update_naxi_relations'), { postfix: '_input' }).NonNull,
					},
					resolve: async (_, args) => {
						const relationsService = new RelationsService({
							accountability: this.accountability,
					...
					args: {
						...
						data: toInputObjectType(
							schemaComposer.createObjectTC({
								name: 'update_naxi_extensions_input',
								fields: {
									meta: schemaComposer.createObjectTC({
										name: 'update_naxi_extensions_input_meta',
										fields: {
											enabled: GraphQLBoolean,
										},
									}),
		...
		if ('naxi_users' in schema.read.collections) {
			schemaComposer.Query.addFields({
				users_me: {
					type: ReadCollectionTypes['naxi_users']!,
		...
		if ('naxi_permissions' in schema.read.collections) {
			schemaComposer.Query.addFields({
				permissions_me: {
		...
		if ('naxi_roles' in schema.read.collections) {
			schemaComposer.Query.addFields({
				roles_me: {
					type: ReadCollectionTypes['naxi_roles']!.List,
					resolve: async (_, args, __, info) => {
						if (!this.accountability?.user && !this.accountability?.role) return null;
		...
		if ('naxi_policies' in schema.read.collections) {
			schemaComposer.Query.addFields({
				policies_me_globals: {
					type: schemaComposer.createObjectTC({
		...
		if ('naxi_users' in schema.update.collections && this.accountability?.user) {
			schemaComposer.Mutation.addFields({
				update_users_me: {
					type: ReadCollectionTypes['naxi_users']!,
					args: {
						data: toInputObjectType(UpdateCollectionTypes['naxi_users']!),
					},
					resolve: async (_, args, __, info) => {
						if (!this.accountability?.user) return null;
						...
						if ('naxi_users' in ReadCollectionTypes) {
							const selections = this.replaceFragmentsInSelections(
								info.fieldNodes[0]?.selectionSet?.selections,
								info.fragments,
							);
		...
		if ('naxi_files' in schema.create.collections) {
			schemaComposer.Mutation.addFields({
				import_file: {
					type: ReadCollectionTypes['naxi_files'] ?? GraphQLBoolean,
					args: {
						url: new GraphQLNonNull(GraphQLString),
						data: toInputObjectType(CreateCollectionTypes['naxi_files']!).setTypeName(
							'create_naxi_files_input',
						),
					},
					resolve: async (_, args, __, info) => {
						...
						if ('naxi_files' in ReadCollectionTypes) {
							const selections = this.replaceFragmentsInSelections(
								info.fieldNodes[0]?.selectionSet?.selections,
								info.fragments,
							);
		...
		if ('naxi_users' in schema.create.collections) {
			schemaComposer.Mutation.addFields({
				users_invite: {
					type: GraphQLBoolean,
					args: {
