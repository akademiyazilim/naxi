...
// TODO Old tests for RolesServices - adapt for PoliciesService
test.todo('unimplemented test');

// import { ForbiddenError, UnprocessableContentError } from '@naxi/errors';
// import { randomUUID } from '@naxi/random';
...
// 	beforeEach(() => {
// 		tracker.on.any('naxi_roles').response({});

// 		tracker.on
// 			.select(/"naxi_roles"."id" from "naxi_roles" order by "naxi_roles"."id" asc limit .*/)
// 			.response([]);

// 		tracker.on
// 			.select(
// 				/(select count\("naxi_users"\."id"\) as "count", "naxi_roles"\."admin_access", "naxi_roles"\."app_access" from "naxi_users").*(where "naxi_roles"\."id" = \?).*/,
// 			)
...
// 							tracker.on.select('select "admin_access" from "naxi_roles"').responseOnce({ admin_access });
// 							tracker.on.select('select "id" from "naxi_users" where "role" = ?').responseOnce([{ id: userId1 }]);
...
// 							tracker.on
// 								.select('select "id" from "naxi_users" where "role" = ?')
// 								.responseOnce([{ id: userId1 }, { id: userId2 }]);

// 							tracker.on.select('select count(*) as "count" from "naxi_users"').responseOnce({ count: 1 });
...
// 							tracker.on.select('select "admin_access" from "naxi_roles"').responseOnce({ admin_access });
// 							tracker.on.select('select "id" from "naxi_users" where "role" = ?').responseOnce([{ id: userId1 }]);
// 							tracker.on.select('select count(*) as "count" from "naxi_users"').responseOnce({ count: 0 });
...
// 							tracker.on
// 								.select('select count(*) as "count" from "naxi_users" where "id" in')
...
// 										sql.startsWith('select count(*) as "count" from "naxi_users"') && bindings.includes(userId2),
// 								)
// 								.response({ count: 1 });
