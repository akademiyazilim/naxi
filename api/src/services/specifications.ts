...
import { spec } from '@naxi/specs';
import { isSystemCollection } from '@naxi/system-data';
import type { Accountability, FieldOverview, Permission, SchemaOverview, Type } from '@naxi/types';
	...
	async generate(host?: string) {
		...
		const spec: OpenAPIObject = {
			...
			servers: [
				{
					url,
					description: 'Your current naxi instance.',
				},
			],
			paths,
		};
