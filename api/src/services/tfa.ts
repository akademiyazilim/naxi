import { InvalidPayloadError } from '@naxi/errors';
...
export class TFAService {
	...
	constructor(options: AbstractServiceOptions) {
		...
		this.itemsService = new ItemsService('naxi_users', options);
	}
	...
	async verifyOTP(key: PrimaryKey, otp: string, secret?: string): Promise<boolean> {
		...
		const user = await this.knex.select('tfa_secret').from('naxi_users').where({ id: key }).first();
	...
	async generateTFA(key: PrimaryKey): Promise<Record<string, string>> {
		const user = await this.knex.select('email', 'tfa_secret').from('naxi_users').where({ id: key }).first();
		...
		const project = await this.knex.select('project_name').from('naxi_settings').limit(1).first();

		return {
			secret,
			url: authenticator.keyuri(user.email, project?.project_name || 'naxi', secret),
		};
	}
