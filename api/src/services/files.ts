...
import { ContentTooLargeError, ForbiddenError, InvalidPayloadError, ServiceUnavailableError } from '@naxi/errors';
import formatTitle from '@naxi/format-title';
import type { BusboyFileStream, File, PrimaryKey, Query } from '@naxi/types';
...
export class FilesService extends ItemsService<File> {
	...
	async uploadOne(
		...
		if ('folder' in payload === false) {
			const settings = await this.knex.select('storage_default_folder').from('naxi_settings').first();
		...
		const sudoFilesItemsService = new ItemsService('naxi_files', {
			knex: this.knex,
			schema: this.schema,
		});
