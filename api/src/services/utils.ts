...
import { systemCollectionRows } from '@naxi/system-data';
import type { Accountability, PrimaryKey, SchemaOverview } from '@naxi/types';
...
export class UtilsService {
	...
	async sort(collection: string, { item, to }: { item: PrimaryKey; to: PrimaryKey }): Promise<void> {
		const sortFieldResponse =
			(await this.knex.select('sort_field').from('naxi_collections').where({ collection }).first()) ||
