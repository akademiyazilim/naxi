...
import type { Column, SchemaInspector } from '@naxi/schema';
import { createInspector } from '@naxi/schema';
import type { Accountability, Field, FieldMeta, RawField, SchemaOverview, Type } from '@naxi/types';
import { addFieldFlag, toArray } from '@naxi/utils';
...
export class FieldsService {
	...
	constructor(options: AbstractServiceOptions) {
		...
		this.itemsService = new ItemsService('naxi_fields', options);
		this.payloadService = new PayloadService('naxi_fields', options);
	...
	async readAll(collection?: string): Promise<Field[]> {
		...
		const nonAuthorizedItemsService = new ItemsService('naxi_fields', {
			knex: this.knex,
			schema: this.schema,
		});
		...
		const aliasQuery = this.knex.select<any[]>('*').from('naxi_fields');
	...
	async readOne(collection: string, field: string): Promise<Record<string, any>> {
		...
		let fieldInfo = await this.knex.select('*').from('naxi_fields').where({ collection, field }).first();
	...
	async createField(
		...
		try {
			const exists =
				...
				isNil(
					await this.knex.select('id').from('naxi_fields').where({ collection, field: field.field }).first(),
				) === false;

			// Check if field already exists, either as a column, or as a row in naxi_fields
			if (exists) {
			...
			await transaction(this.knex, async (trx) => {
				const itemsService = new ItemsService('naxi_fields', {
					knex: trx,
					accountability: this.accountability,
					schema: this.schema,
				});
		...
		try {
			...
			await transaction(this.knex, async (trx) => {
				...
				if (Object.keys(collectionMetaUpdates).length > 0) {
					await trx('naxi_collections').update(collectionMetaUpdates).where({ collection });
				}
				...
				if (metaRow) {
					...
					await trx('naxi_fields')
						.update({ group: null })
						.where({ group: metaRow.field, collection: metaRow.collection });
				}

				await trx('naxi_fields').delete().where({ collection, field });
			});
