...
import { ForbiddenError, InvalidPayloadError, LimitExceededError, UnprocessableContentError } from '@naxi/errors';
import type { ApiOutput, BundleExtension, ExtensionSettings } from '@naxi/extensions';
import { describe, type DescribeOptions } from '@naxi/extensions-registry';
import type { Accountability, DeepPartial, SchemaOverview } from '@naxi/types';
import { isObject } from '@naxi/utils';
...
export class ExtensionsService {
	...
	constructor(options: AbstractServiceOptions) {
		...
		this.extensionsItemService = new ItemsService('naxi_extensions', {
			knex: this.knex,
			schema: this.schema,
	...
	private async checkBundleAndSyncStatus(trx: Knex, extensionId: string, extension: ApiOutput) {
		...
		const hasEnabledChildren = !!(await trx('naxi_extensions')
			.where({ bundle: parentId })
			.where({ enabled: true })
			.first());

		if (hasEnabledChildren) {
			await trx('naxi_extensions').update({ enabled: true }).where({ id: parentId });
		} else {
			await trx('naxi_extensions').update({ enabled: false }).where({ id: parentId });
		}
	}
}
