...
import type { ForeignKey, SchemaInspector } from '@naxi/schema';
import { createInspector } from '@naxi/schema';
import { systemRelationRows } from '@naxi/system-data';
import type { Accountability, Query, Relation, RelationMeta, SchemaOverview } from '@naxi/types';
...
export class RelationsService {
	...
	constructor(options: AbstractServiceOptions) {
		...
		this.relationsItemService = new ItemsService('naxi_relations', {
			knex: this.knex,
			schema: this.schema,
			// We don't set accountability here. If you have read access to certain fields, you are
			// allowed to extract the relations regardless of permissions to naxi_relations. This
	...
	async createOne(relation: Partial<Relation>, opts?: MutationOptions): Promise<void> {
		...
		try {
			...
			await transaction(this.knex, async (trx) => {
				...
				const relationsItemService = new ItemsService('naxi_relations', {
					knex: trx,
					schema: this.schema,
					// We don't set accountability here. If you have read access to certain fields, you are
					// allowed to extract the relations regardless of permissions to naxi_relations. This
	...
	async deleteOne(collection: string, field: string, opts?: MutationOptions): Promise<void> {
		...
		try {
			await transaction(this.knex, async (trx) => {
				...
				if (existingRelation.meta) {
					await trx('naxi_relations').delete().where({ many_collection: collection, many_field: field });
				}
	...
	/**
	 * Combine raw schema foreign key information with naxi relations meta rows to form final
	 * Relation objects
	 */
