...
import { ForbiddenError, InvalidPayloadError, RecordNotUniqueError } from '@naxi/errors';
import type { Item, PrimaryKey, RegisterUserInput, User } from '@naxi/types';
import { getSimpleHash, toArray, validatePayload } from '@naxi/utils';
import { FailedValidationError, joiValidationErrorItemToErrorExtensions } from '@naxi/validation';
import Joi from 'joi';
...
export class UsersService extends ItemsService {
	 ...
	 * naxi_settings.auth_password_policy
	 */
	...
	override async deleteMany(keys: PrimaryKey[], opts: MutationOptions = {}): Promise<PrimaryKey[]> {
		...
		// Manual constraint, see https://github.com/naxi/naxi/pull/19912
		await this.knex('naxi_comments').update({ user_updated: null }).whereIn('user_updated', keys);
		await this.knex('naxi_notifications').update({ sender: null }).whereIn('sender', keys);
		await this.knex('naxi_versions').update({ user_updated: null }).whereIn('user_updated', keys);
	...
	async requestPasswordReset(email: string, url: string | null, subject?: string | null): Promise<void> {
		...
		const token = jwt.sign(payload, getSecret(), { expiresIn: '1d', issuer: 'naxi' });
	...
	async resetPassword(token: string, password: string): Promise<void> {
		const { email, scope, hash } = jwt.verify(token, getSecret(), { issuer: 'naxi' }) as {
			email: string;
			scope: string;
			hash: string;
		};
