...
import { randomInteger, randomUUID } from '@naxi/random';
import { Driver, StorageManager } from '@naxi/storage';
	...
	describe('Services / Files', () => {
		...
		describe('uploadOne', () => {
			...
			beforeEach(() => {
				...
				tracker.on.select('select "storage_default_folder" from "naxi_settings"').response([]);
			...
			it('should set the `uploaded_on` field to the current date', async () => {
				tracker.on
					.select(
						'select "folder", "filename_download", "filename_disk", "title", "description", "metadata" from "naxi_files" where "id" = ?',
					)
					.response(null);
