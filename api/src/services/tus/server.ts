...
import type { Driver, TusDriver } from '@naxi/storage';
import { supportsTus } from '@naxi/storage';
...
export async function createTusServer(context: Context): Promise<[Server, () => void]> {
	...
	const server = new Server({
		...
		async onUploadFinish(req: any, res, upload) {
			const service = new ItemsService<File>('naxi_files', {
				schema: req.schema,
			});
	...
	server.on(EVENTS.POST_CREATE, async (_req, res, upload) => {
		res.setHeader('naxi-File-Id', upload.metadata!['id']!);
	});
