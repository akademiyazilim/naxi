...
import type { TusDriver, ChunkedUploadContext } from '@naxi/storage';
import type { Accountability, File, SchemaOverview } from '@naxi/types';
...
export class TusDataStore extends DataStore {
	...
	public override async create(upload: Upload): Promise<Upload> {
		...
		const filesItemsService = new ItemsService<File>('naxi_files', {
		...
		if ('folder' in fileData === false) {
			const settings = await knex.select('storage_default_folder').from('naxi_settings').first();
	...
	public override async write(readable: stream.Readable, tus_id: string, offset: number): Promise<number> {
		...
		const sudoFilesItemsService = new ItemsService<File>('naxi_files', {
			schema: this.schema,
		});
