...
import type { Accountability, File, Query, SchemaOverview } from '@naxi/types';
import { parseJSON, toArray } from '@naxi/utils';
import { createTmpFile } from '@naxi/utils/node';
...
export class ExportService {
	...
	async exportToFile(
		...
		const { createTmpFile } = await import('@naxi/utils/node');
		const tmpFile = await createTmpFile().catch(() => null);
