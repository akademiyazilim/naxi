...
export default async function init(): Promise<void> {
	...
	await db('naxi_roles').insert({ ...defaultAdminRole, id: role });
	await db('naxi_policies').insert({ ...defaultAdminPolicy, id: policy });
	await db('naxi_access').insert({ id: randomUUID(), role, policy });

	await db('naxi_users').insert({
		...defaultAdminUser,
		id: randomUUID(),
	...
	process.stdout.write(`\nStart naxi by running:\n`);
	process.stdout.write(`  ${chalk.blue('cd')} ${rootPath}\n`);
	process.stdout.write(`  ${chalk.blue('npx naxi')} start\n`);
