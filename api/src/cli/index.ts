...
export async function createCli(): Promise<Command> {
	...
	program.name('naxi').usage('[command] [options]');
	program.version(version, '-v, --version');

	program.command('start').description('Start the naxi API').action(startServer);
	program.command('init').description('Create a new naxi Project').action(init);
