...
import type {
	...
	HybridExtension,
	OperationApiConfig,
} from '@naxi/extensions';
import { APP_SHARED_DEPS, HYBRID_EXTENSION_TYPES } from '@naxi/extensions';
import { generateExtensionsEntrypoint } from '@naxi/extensions/node';
import type {
	ActionHandler,
	EmbedHandler,
	FilterHandler,
...
import { isTypeIn, toBoolean } from '@naxi/utils';
import { pathToRelativeUrl, processId } from '@naxi/utils/node';
...
export class ExtensionManager {
	...
	private async registerSandboxedApiExtension(extension: ApiExtension | HybridExtension) {
		...
		await module.instantiate(context, (specifier) => {
			if (specifier !== 'naxi:api') {
				throw new Error('Imports other than "naxi:api" are prohibited in API extension sandboxes');
			}
