import { type naxiError, ForbiddenError } from '@naxi/errors';

export function createCollectionForbiddenError(path: string, collection: string): naxiError<any> {
...
export function createFieldsForbiddenError(path: string, collection: string, fields: string[]): naxiError<any> {
	const pathSuffix = path === '' ? 'root' : `"${path}"`;
