...
import { InvalidPayloadError, ServiceUnavailableError } from '@naxi/errors';
...
export default async function createApp(): Promise<express.Application> {
	...
	if ((await isInstalled()) === false) {
		logger.error(`Database doesn't have naxi tables installed.`);
		process.exit(1);
	}
	...
	app.use((_req, res, next) => {
		res.setHeader('X-Powered-By', 'naxi');
		next();
	});
	...
	if (env['SERVE_APP']) {
		const adminPath = require.resolve('@naxi/app');
		...
		const htmlWithVars = html
			...
			.replace('<!-- naxi-embed-head -->', embeds.head)
			.replace('<!-- naxi-embed-body -->', embeds.body);
